...
for epoch in range(1, config.max_epoch+1):
    ...
    for batch_idx, (batch, keyword_batch) in enumerate(zip(DataLoader(dev_set, batch_size=config.eval_batch_size, 
        ...
        dev_batch = [{
            # 'input': keyword_pred_text[i] + ' \n ' + batch.input_text[i][batch.input_text[i].find('\n')+1:].strip(),
            'input': batch.input_text[i][:batch.input_text[i].rfind('\n', 0, batch.input_text[i].rfind('\n'))].strip() + ' \n ' + \
                keyword_pred_text[i] + ' \n ' + batch.input_text[i][batch.input_text[i].find('\n', batch.input_text[i].find('\n')+1)+1:].strip(),
            'target': batch.target_text[i],
            # 'info': batch.infos[i] + (keyword_batch.target_text[i] + ' \n ' + batch.input_text[i][batch.input_text[i].find('\n')+1:].strip(), )
            'info': batch.infos[i] + (batch.input_text[i][:batch.input_text[i].rfind('\n', 0, batch.input_text[i].rfind('\n'))].strip() + ' \n ' + \
                keyword_batch.target_text[i] + ' \n ' + batch.input_text[i][batch.input_text[i].find('\n', batch.input_text[i].find('\n')+1)+1:].strip(), )
        } for i in range(len(keyword_pred_text))]
        batch = dev_set.collate_fn(dev_batch)
    ...
    dev_scores = {
        'keyword_id': compute_f1(dev_pred_key_num, dev_gold_key_num, dev_match_key_num),
    ...
    logger.info('Keyword I  - P: {:5.2f} ({:4d}/{:4d}), R: {:5.2f} ({:4d}/{:4d}), F: {:5.2f}'.format(
        dev_scores['keyword_id'][0] * 100.0, dev_match_key_num, dev_pred_key_num, 
        dev_scores['keyword_id'][1] * 100.0, dev_match_key_num, dev_gold_key_num, dev_scores['keyword_id'][2] * 100.0))
    logger.info("---------------------------------------------------------------------")
    ...
    if best_dev_flag:    
        ...
        for batch_idx, (batch, keyword_batch) in enumerate(zip(DataLoader(test_set, batch_size=config.eval_batch_size, 
            ...
            batch = test_set.collate_fn(test_batch)
            for i_text, g_text, p_text, info, keyword_info in zip(keyword_input_text, keyword_gold_text, keyword_pred_text, batch.infos, keyword_batch.infos):
                theclass = getattr(sys.modules[template_file], info[1].replace(':', '_').replace('-', '_'), False)
                assert theclass
                template = theclass(config.input_style, config.output_style, info[2], info[1], info[0])
        ...
        logger.info("---------------------------------------------------------------------")
        logger.info('Keyword I  - P: {:5.2f} ({:4d}/{:4d}), R: {:5.2f} ({:4d}/{:4d}), F: {:5.2f}'.format(
            test_scores['keyword_id'][0] * 100.0, test_match_key_num, test_pred_key_num, 
            test_scores['keyword_id'][1] * 100.0, test_match_key_num, test_gold_key_num, test_scores['keyword_id'][2] * 100.0))
        logger.info("---------------------------------------------------------------------")
        logger.info('Trigger I  - P: {:5.2f} ({:4d}/{:4d}), R: {:5.2f} ({:4d}/{:4d}), F: {:5.2f}'.format(
