    ...
    for batch in DataLoader(dev_set, batch_size=config.eval_batch_size, shuffle=False, collate_fn=dev_set.collate_fn):
        ...
        for event_type in vocab['event_type_itos']:
            ...
            for tokens in batch.tokens:
                ...
                keyword_inputs.append(template.generate_keywords_input_str())
            keyword_inputs = tokenizer(keyword_inputs, return_tensors='pt', padding=True, max_length=config.max_length)
            keyword_enc_idxs = keyword_inputs['input_ids'].cuda()
            keyword_enc_attn = keyword_inputs['attention_mask'].cuda()
            keyword_outputs = model.model.generate(input_ids=keyword_enc_idxs, attention_mask=keyword_enc_attn, num_beams=config.beam_size, max_length=config.max_output_length)
            keyword_final_outputs = [tokenizer.decode(output, skip_special_tokens=True, clean_up_tokenization_spaces=True) for output in keyword_outputs]
            
            inputs = []
            for i, tokens in enumerate(batch.tokens):
                template = theclass(config.input_style, config.output_style, tokens, event_type)
                input_text = template.generate_input_str('')
                input_text = input_text[:input_text.rfind('\n', 0, input_text.rfind('\n'))].strip() + ' \n ' + \
                    keyword_final_outputs[i] + ' \n ' + input_text[input_text.find('\n', input_text.find('\n')+1)+1:].strip()
